...
function reasonableTime(appOrParcel, lifecycle) {
  ...
  return new Promise(function (resolve, reject) {
    var finished = false;
    var errored = false;
    // /这里就是调用了生命周期的函数,同时可以看到调用的时候传入了一个参数,这个参数就是从主应用中传入子应用生命周期函数的prop
...
function toLoadPromise(app) {
  return Promise.resolve().then(function () {
    //在registerApplication返回的json对象 没有loadPromise属性
    if (app.loadPromise) {
    ...
    return app.loadPromise = Promise.resolve().then(function () {
      // appNameOrConfig.app 就是我们regist 时传入的加载子应用的函数
      var loadPromise = app.loadApp(getProps(app));
      // 校验传入register的第二个参数返回的是不是promise
...
// 当用户在执行 pushState方法的时候也能够重新reroute
function patchedUpdateState(updateState, methodName) {
  return function () {
    // 获取跳转之前的页面 url
    var urlBefore = window.location.href;
    //劫持使用原生pushState方法，这里就保证了原来的 pushState 功能不会丢失
...
　　 //这个urlRerouteOnly是官方文档上可以传入start( urlRerouteOnly: true)函数的参数
    //如果说你在start时候传入了这个参数，那么如果当你调用pushState（且调用pushState并不想改变url，只是想达到你某个目的）
    // 那么此时，前后url没发生改变那么就不会重新reroute， 一定程度上提升了性能。
    if (!urlRerouteOnly || urlBefore !== urlAfter) {
      // 这里本质上  定义了一个 popstate 事件并触发，回调之后就执行了reroute
      if (isStarted()) {
...
function createPopStateEvent(state, originalMethodName) {
  ...
  var evt;
  // 这里自定义了一个popstate事件，并且返回
  // 在上层的window.dispatch去触发这个事件的时候本质上就是触发popstate事件
  // 那么我们在上面源码的开头就已经监听了popstate事件，监听到了的回调函数就是执行reroute
...
function getAppChanges() {
  ...
  apps.forEach(function (app) {
    // 判断当前的url需要激活哪一个子应用  取 app.activeWhen
    ...
    switch (app.status) {
      ...
      case NOT_LOADED: // 在执行 registerApplication 时，默认app.status都是设置为 NOT_LOADED的， 如果匹配成功的话就把app推入appsLoad数组中，表明这个子应用即将被加载
...
function reroute() {
  ...
  if (isStarted()) {//  start() 方法开头会 设置为true
  ...
  function performAppChanges() {
    return Promise.resolve().then(function () {
      ...
      // window.dispatchEvent注册了绑定了一些自定义事件  给开发者调用
...
function tryToBootstrapAndMount(app, unmountAllPromise) {
  ...
  if (shouldBeActive(app)) {
    // 如果确认了我们要渲染这个子应用那么调用toBootstrapPromise函数
